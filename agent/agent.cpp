...
int main(int argc, char *argv[]) {
    ...
    std::cout << "--- Agent v7.5 (SSL Fixed) ---" << std::endl;
    ...
    while (true) {
        ...
        if (connect(sockGlobal, (sockaddr *)&addr, sizeof(addr)) == 0) {
            ...
            char buf[4096]; SSL_read(sslGlobal, buf, sizeof(buf)); 
            send_ws_text("40"); 
            if (SSL_read(sslGlobal, buf, sizeof(buf)) > 0) {
                isConnected = true;
                ...
                while (isConnected) {
                    if (!targetViewerId.empty()) {
                        ...
                        json update; update["senderId"] = agentUserId; update["targetId"] = targetViewerId; 
